...
exec docker run --rm \
    ...
    ${MAINTENANCE_BATCH_SIZE:+-e MAINTENANCE_BATCH_SIZE="${MAINTENANCE_BATCH_SIZE}"} \
    ${ARCHIVE_CRF:+-e ARCHIVE_CRF="${ARCHIVE_CRF}"} \
    ${SCAN_INTERVAL_SECONDS:+-e SCAN_INTERVAL_SECONDS="${SCAN_INTERVAL_SECONDS}"} \
